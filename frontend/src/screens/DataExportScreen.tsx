...
const DataExportScreen = () => {
  ...
  const dairyExpenseData = useSelector(
    (state: RootState) => state.dairyExpense.entries,
  );
  const otherExpenseData = useSelector(
    (state: RootState) => state.otherExpense.items,
  );
  ...
  const availableYears = React.useMemo(() => {
    const years = [...new Set(farms.map((f) => f.year))].sort((a, b) =>
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      doc.text("ShetiHishob Report", pageWidth / 2, 20, { align: "center" });

      doc.setFontSize(10);
      doc.setTextColor(100);
      doc.text(`Year: ${selectedYear}`, pageWidth / 2, 28, { align: "center" });
      doc.text(
        `Exported: ${format(new Date(), "dd MMM yyyy, hh:mm a")}`,
        pageWidth / 2,
        33,
        { align: "center" },
      );
      ...
      if (selectedModule === "all") {
        ...
        if (name) {
          ...
          const farmRows = farms
            .filter((f) => String(f.year) === String(selectedYear))
            .map((f) => [
      ...
      const label =
        MODULES.find((m) => m.id === selectedModule)?.label.replace(
          /\s+/g,
          "",
        ) || "Report";
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const createSheetData = (moduleName: string) => {
        ...
        switch (moduleName) {
          ...
          case "Other_Expenses":
            ...
            break;
          case "Farms":
            rawData = farms.filter(
              (f) => String(f.year) === String(selectedYear),
            );
            mapFn = (i) => ({
              "Farm Name": i.name || "-",
              Year: i.year || "-",
      ...
      const appendSheetWithCalculations = (sheetName: string, data: any[]) => {
        ...
        if (!worksheet)
          throw new Error(`Worksheet generation failed for ${sheetName}`);

        // Set column widths
        worksheet["!cols"] = keys.map((k) => ({
          wch: Math.max(k.length + 8, 15),
        }));
      ...
      if (!hasExportedAny) {
        toast.error("No records found for the selected year and module.", {
          id: toastId,
        });
        return;
      }
  ...
  return (
    <div className="max-w-md mx-auto space-y-6 pb-10">
      <div className="flex items-center gap-4 mb-2">
        ...
        <div>
          <h1 className="text-2xl font-black uppercase text-foreground leading-none">
            Data Export
          </h1>
          <p className="text-[10px] font-black uppercase text-muted-foreground mt-1 tracking-widest leading-none">
            Settings → Export
          </p>
        </div>
      </div>
          ...
          <div className="space-y-2">
            ...
            {availableYears.length > 0 ? (
              ...
              <div className="p-4 bg-muted/50 rounded-2xl border border-dashed border-border text-center">
                <p className="text-xs font-black uppercase text-muted-foreground py-2">
                  No farm years available
                </p>
              </div>
            )}
          </div>
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="text-left">
                <span className="block text-white font-black uppercase tracking-widest text-sm">
                  Download PDF
                </span>
                <span className="block text-[9px] text-white/70 font-bold uppercase tracking-tight">
